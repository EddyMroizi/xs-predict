...
- ξ (forme) = 0.44 — queue épaisse, cohérent avec la réassurance catastrophe
- σ (échelle) = 72 856 € — calibré sur les excédents observés
- λ (Poisson) = 116.6 sinistres XS / an
...
La fréquence et la sévérité ont des drivers différents. La fréquence dépend de l'exposition (zone géographique, branche, historique). La sévérité dépend de la valeur assurée et du type de sinistre. Un modèle unique mélangerait ces deux logiques.
...
- Perte Huber (α=0.9) → robuste aux sinistres extrêmes
...
- GPD calibrée : ξ = 0.4, σ = 80 000 € (valeurs issues de la littérature Cat)
